...
/// useActionState) termina — identificado pelo carimbo `ts`, que muda a cada
/// execução mesmo quando a mensagem se repete.
export function useActionToast(state: ActionState) {
  ...
  useEffect(() => {
    ...
    if (state.ok) {
      ...
      toast.error(state.message || "Não foi possível concluir.");
    }
  }, [state]);
}
